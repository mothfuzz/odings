...
async function runWasm(wasmPath, consoleElement, setupExtraForeignImports) {
	let wasmMemoryInterface = new WasmMemoryInterface();

	let imports = odinSetupDefaultImports(wasmMemoryInterface, consoleElement);
	let exports = {};

	if (setupExtraForeignImports !== undefined) {
		let extraForeignImports = setupExtraForeignImports(wasmMemoryInterface);
		imports = {
			...imports,
			...extraForeignImports,
		};
